...
def test_augment_produces_8x_count():
    ...
    train_set, _, _ = split_data_to_tensors(data, train_size=0.8, val_size=0.1, test_size=0.1)
...
def test_augment_no_duplicate_tensors():
    ...
    for i in range(64):
        for j in range(i + 1, 64):
            assert not np.allclose(flat[i], flat[j]), f"Augmented images {i} and {j} are identical"
